...
export class JustClient extends SapphireClient {
  ...
  private async _init() {
    ...
    this.audio.shoukaku.on("ready", (name, resumed) => this.logger.info(`[Plugin#Audio:Ready] Lavalink Node: ${name} is now connected. This connection is ${resumed ? "resumed" : "a new connection"}.`));
    this.audio.shoukaku.on("error", (name, error) => this.logger.error(`[Plugin#Audio:Error] Lavalink Node: ${name} emitted an error. ${error.stack}`));
    this.audio.shoukaku.on("close", (name, code, reason) => this.logger.warn(`[Plugin#Audio:Warn] Lavalink Node: ${name} closed with code ${code}. Reason: ${reason || "No reason."}`));
    ...
    // });
    this.audio.shoukaku.on("debug", (name, data) => this.logger.debug(`[Plugin#Audio:Debug] Lavalink Node: ${name} - Data: ${JSON.stringify(data)}`));
