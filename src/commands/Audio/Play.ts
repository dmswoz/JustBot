...
export class Play extends Command {
    public constructor(context: Command.LoaderContext, options: Command.Options) {
        super(context, {
            ...options,
            fullCategory: ["Audio"],
            requiredClientPermissions: ["SendMessages", "Connect", "Speak", "EmbedLinks"]
        });
    }
    ...
    public async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
        ...
        if(!this.container.client.audio.getPlayer(interaction.guildId!)) {
            ...
            if(!voice.channel) return interaction.reply({
        ...
        const player = await this.container.client.audio.getPlayer(interaction.guildId!);
        ...
        if(searchResult.type === "PLAYLIST") {
            if(player!.queue.length <= 0) {
                if(player!.playing || player!.paused) {
                    interaction.editReply(await Messages.getPlaylistQueueEmbed(searchResult, player!));
                } else interaction.editReply(await Messages.getPlaylistEmbed(searchResult));
            } else interaction.editReply(await Messages.getPlaylistQueueEmbed(searchResult, player!));

            for(const track of searchResult.tracks) player?.queue.add(track);
        } else if(searchResult.type === "SEARCH") {
            if(player!.queue.length <= 0) {
                if(player!.playing || player!.paused) {
                    interaction.editReply(await Messages.getTrackQueueEmbed(searchResult, player!, interaction.user));
                } else interaction.editReply(await Messages.getTrackEmbed(searchResult, interaction.user));
            } else interaction.editReply(await Messages.getTrackQueueEmbed(searchResult, player!, interaction.user));
